...
namespace I2.Loc
{
	...
	public class Localize : MonoBehaviour
	{
		...
		public void OnLocalize(bool Force = false)
		{
			...
			MainTranslation = ((!string.IsNullOrEmpty(FinalTerm) && !(FinalTerm == "-")) ? LocalizationManager.GetTranslation(FinalTerm, false) : null);
			SecondaryTranslation = ((!string.IsNullOrEmpty(FinalSecondaryTerm) && !(FinalSecondaryTerm == "-")) ? LocalizationManager.GetTranslation(FinalSecondaryTerm, false) : null);
		...
		{
 string secondary; string value;			DeserializeTranslation(mainTranslation, out value, out secondary);
			T val = (T)null;
