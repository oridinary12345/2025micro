	...
	public class TranslationJob_WEB : TranslationJob_WWW
	{
		...
		private void FindAllQueries()
		{
			...
			foreach (KeyValuePair<string, TranslationQuery> request in _requests)
			{
				...
				string[] targetLanguagesCode = value.TargetLanguagesCode;
				foreach (string str in targetLanguagesCode)
				{
					List<KeyValuePair<string, string>> list = mQueries;
					TranslationQuery value2 = request.Value;
					string origText = value2.OrigText;
					TranslationQuery value3 = request.Value;
					list.Add(new KeyValuePair<string, string>(origText, value3.LanguageCode + ":" + str));
				}
			}
		...
		public void ProcessResult(byte[] bytes, string errorMsg)
		{
			if (string.IsNullOrEmpty(errorMsg))
			{
				...
				string message = ParseTranslationResult(@string, "aab");
