...
namespace PlayFab.Json
{
	...
	public class PocoJsonSerializerStrategy : IJsonSerializerStrategy
	{
		...
		{
			object[] customAttributes = memberInfo.GetCustomAttributes(typeof(JsonProperty), true);
		...
		public virtual object DeserializeObject(object value, Type type)
		{
			...
			if (text != null)
			{
				if (text.Length != 0)
				{
					...
					if (type == typeof(Uri))
					{
 Uri result2;						if (Uri.IsWellFormedUriString(text, UriKind.RelativeOrAbsolute) && Uri.TryCreate(text, UriKind.RelativeOrAbsolute, out result2))
						{
							return result2;
						}
						return null;
			...
			if (dictionary != null)
			{
				...
				else
				{
					...
					{
 object value2;						if (dictionary2.TryGetValue(item2.Key, out value2))
						{
		...
		protected virtual bool TrySerializeUnknownTypes(object input, out object output)
		{
			...
			foreach (KeyValuePair<MemberInfo, ReflectionUtils.GetDelegate> item in dictionary2)
			{
				if (item.Value != null)
				{
 JsonProperty jsonProp; string jsonName;					MapClrMemberNameToJsonFieldName(item.Key, out jsonName, out jsonProp);
