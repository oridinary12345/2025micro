...
namespace PlayFab.Json
{
	...
	public static class PlayFabSimpleJson
	{
		...
		{
 object obj;			if (TryDeserializeObject(json, out obj))
			{
				return obj;
			}
 ...
 uint result;					if (!(success = uint.TryParse(json.Substring(index, 4), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out result)))
					{
						return string.Empty;
					}
		...
		private static object ParseNumber(string json, ref int index, ref bool success)
		{
			...
			{
 double result;				success = double.TryParse(json.Substring(index, length), NumberStyles.Any, CultureInfo.InvariantCulture, out result);
				result2 = result;
			}
			else if (text.IndexOf("-", StringComparison.OrdinalIgnoreCase) == -1)
			{
 ulong result3;				success = ulong.TryParse(json.Substring(index, length), NumberStyles.Any, CultureInfo.InvariantCulture, out result3);
				result2 = result3;
			}
			else
			{
 long result4;				success = long.TryParse(json.Substring(index, length), NumberStyles.Any, CultureInfo.InvariantCulture, out result4);
				result2 = result4;
			}
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						{
 object output;							flag = jsonSerializerStrategy.TrySerializeNonPrimitiveObject(value, out output);
							if (flag)
							{
