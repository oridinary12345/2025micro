...
namespace TMPro
{
 ...
 TMP_Glyph glyph;				fontAssetForWeight = TMP_FontUtilities.SearchForGlyph(m_currentFontAsset, num2, out glyph);
				if (glyph == null)
				{
					...
					{
						int spriteIndex = -1;
						spriteAsset = TMP_SpriteAsset.SearchForSpriteByUnicode(spriteAsset, num2, true, out spriteIndex);
						if (spriteIndex != -1)
						{
		...
		protected override void GenerateTextMesh()
		{
			...
			for (int i = 0; i < m_char_buffer.Length && m_char_buffer[i] != 0; i++)
			{
				...
				vector2.y = num22 + (m_cached_TextElement.yOffset + num5 + a.yPlacement) * num2 - m_lineOffset + m_baselineOffset;
				vector2.z = 0f;
				vector3.x = vector2.x;
				...
				if (m_textElementType == TMP_TextElementType.Character && !isUsingAlternateTypeface && ((m_style & FontStyles.Italic) == FontStyles.Italic || (m_fontStyle & FontStyles.Italic) == FontStyles.Italic))
				{
					...
					Vector3 b = new Vector3(num23 * ((m_cached_TextElement.yOffset + num5 + num6) * num2), 0f, 0f);
					Vector3 b2 = new Vector3(num23 * ((m_cached_TextElement.yOffset - m_cached_TextElement.height - num5 - num6) * num2), 0f, 0f);
					vector2 += b;
					vector3 += b2;
					vector4 += b;
					vector5 += b2;
				...
				else if (!m_isRightToLeft)
				{
					...
					m_xAdvance += ((m_cached_TextElement.xAdvance * num36 * num7 + m_characterSpacing + m_currentFontAsset.normalSpacingOffset + a.xAdvance) * num2 + m_cSpacing) * (1f - m_charWidthAdjDelta);
					if (char.IsWhiteSpace((char)num4) || num4 == 8203)
					{
						m_xAdvance += m_wordSpacing * num2;
					}
			...
			float num44 = m_previousLossyScaleY = lossyScale.y;
			...
			for (int j = 0; j < m_characterCount; j++)
			{
				...
				case TextAlignmentOptions.MidlineLeft:
				case TextAlignmentOptions.CaplineLeft:
					vector6 = (m_isRightToLeft ? new Vector3(0f - tMP_LineInfo.maxAdvance, 0f, 0f) : new Vector3(tMP_LineInfo.marginLeft, 0f, 0f));
					break;
				case TextAlignmentOptions.Top:
				case TextAlignmentOptions.Center:
				case TextAlignmentOptions.Bottom:
				...
				case TextAlignmentOptions.MidlineRight:
				case TextAlignmentOptions.CaplineRight:
					vector6 = (m_isRightToLeft ? new Vector3(tMP_LineInfo.marginLeft + tMP_LineInfo.width, 0f, 0f) : new Vector3(tMP_LineInfo.marginLeft + tMP_LineInfo.width - tMP_LineInfo.maxAdvance, 0f, 0f));
					break;
				case TextAlignmentOptions.TopJustified:
				case TextAlignmentOptions.TopFlush:
				case TextAlignmentOptions.Justified:
				...
				case TextAlignmentOptions.CaplineFlush:
				{
					...
					if ((!char.IsControl(character4) && lineNumber < m_lineNumber) || flag12 || tMP_LineInfo.maxAdvance > tMP_LineInfo.width)
					{
						...
						vector6 = ((character3 != '\u00a0' && (character3 == '\t' || char.IsSeparator(character3))) ? (m_isRightToLeft ? (vector6 - new Vector3(num53 * (1f - num56) / (float)num55, 0f, 0f)) : (vector6 + new Vector3(num53 * (1f - num56) / (float)num55, 0f, 0f))) : (m_isRightToLeft ? (vector6 - new Vector3(num53 * num56 / (float)num54, 0f, 0f)) : (vector6 + new Vector3(num53 * num56 / (float)num54, 0f, 0f))));
					}
					else
					{
						vector6 = (m_isRightToLeft ? new Vector3(tMP_LineInfo.marginLeft + tMP_LineInfo.width, 0f, 0f) : new Vector3(tMP_LineInfo.marginLeft, 0f, 0f));
					}
					break;
				}
				}
				...
				if (isVisible)
				{
					...
					case TMP_TextElementType.Character:
					{
						...
						case TextureMappingOptions.Line:
							...
							{
								characterInfo[j].vertex_BL.uv2.x = (characterInfo[j].vertex_BL.position.x + vector6.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num57;
								characterInfo[j].vertex_TL.uv2.x = (characterInfo[j].vertex_TL.position.x + vector6.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num57;
								characterInfo[j].vertex_TR.uv2.x = (characterInfo[j].vertex_TR.position.x + vector6.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num57;
								characterInfo[j].vertex_BR.uv2.x = (characterInfo[j].vertex_BR.position.x + vector6.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num57;
							}
							break;
				...
				if (flag14)
				{
					...
					if (!flag2 && flag15 && j <= tMP_LineInfo.lastVisibleCharacterIndex && character3 != '\n' && character3 != '\r' && (j != tMP_LineInfo.lastVisibleCharacterIndex || !char.IsSeparator(character3)))
					{
						...
						b4 = m_textInfo.characterInfo[j].baseLine;
					}
					...
					else if (flag2 && j < m_characterCount && (m_textInfo.characterInfo[j + 1].pointSize != num51 || !TMP_Math.Approximately(m_textInfo.characterInfo[j + 1].baseLine + zero3.y, b4)))
					{
						flag2 = false;
						int lastVisibleCharacterIndex3 = tMP_LineInfo.lastVisibleCharacterIndex;
						zero2 = ((j > lastVisibleCharacterIndex3) ? new Vector3(m_textInfo.characterInfo[lastVisibleCharacterIndex3].topRight.x, m_textInfo.characterInfo[lastVisibleCharacterIndex3].baseLine + strikethrough * num52, 0f) : new Vector3(m_textInfo.characterInfo[j].topRight.x, m_textInfo.characterInfo[j].baseLine + strikethrough * num52, 0f));
						DrawUnderlineMesh(start2, zero2, ref index, num52, num52, num52, num45, underlineColor);
					}
